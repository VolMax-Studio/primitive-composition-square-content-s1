...
expected_a_sha="5b97b0a5eea88415f98783443c4f15fc3b4d1030fd3b0e99826110795a0216f5"
expected_mathlib_rev="5ed2965256430c3649e86755f9576b54eca72435"

actual_a_sha="$(sha256sum PrimitiveTripleFilter.lean | cut -d' ' -f1)"
if [[ "$actual_a_sha" != "$expected_a_sha" ]]; then
  echo "FAIL: frozen A hash mismatch" >&2
  exit 1
fi
...
proof_files=(
  PrimitiveCompositionSquareContent.lean
  YZeroTest.lean
  SignatureAudit.lean
  AxiomAudit.lean
)
forbidden_pattern='(^|[^[:alnum:]_])sorry([^[:alnum:]_]|$)|native_decide|^[[:space:]]*axiom([[:space:]]|$)'
...
case "$forbidden_rc" in
  0)
    printf '%s\n' "$forbidden_output" >&2
    echo "FAIL: forbidden proof escape found in B files" >&2
    exit 1
    ;;
  1)
    ;;
...
echo "PASS: primitive-composition-square-content-s1"
